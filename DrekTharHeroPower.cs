...
// Type: BobsBuddy.HeroPowers.DrekTharHeroPower
...
public class DrekTharHeroPower(
  string cardId,
  Simulator simulator,
  ...
  HeroPowerData data) : HeroPower(cardId, simulator, controlledByPlayer, data), IOnStartOfCombat, IEntity, IOnAfterAttackStep
{
  ...
  private void CheckForSummon()
  {
    if (this.FriendlySide.Count >= 7 || this._tokenSummoned || this.Simulator.Turn < 7)
      return;
    Minion highestAttackMinion = this.Simulator.GetHighestAttackMinion(this.FriendlySide);
    if (highestAttackMinion == null)
      return;
    this.Simulator.TrySummonMinion(new Summon(highestAttackMinion.Clone()), this.FriendlySide, highestAttackMinion.BoardPosition() + 1, (Entity) this);
